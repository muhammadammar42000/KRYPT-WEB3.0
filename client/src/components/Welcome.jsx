...
export default function Welcome() {
  ...
  return (
    <div className='flex w-full justify-center items-center'>
      <div className='flex md:flex-row flex-col items-start justify-between md:p-20 px-5'>
        <div className='flex flex-1 justify-start flex-col md:mr-10 h-screen'>
          ...
          {!currentAccount && (
            <button type='button' onClick={connectWallet} className='flex flex-row justify-center items-center my-5 bg-[#2952e3] p-3 rounded-full cursor-pointer hover:bg-[#2546bd]'>
              <p className='text-white text-base flex font-semibold'>CONNECT WALLET <IoMdWallet className='ml-5 ' style={{ fontSize: '22px' }} /> </p>
            </button>
          )}
        ...
        <div className="flex flex-col flex-1 items-center justify-start w-full mf:mt-0 mt-10">
          ...
          <div className="p-5 sm:w-96 w-full flex flex-col justify-start items-center blue-glassmorphism">
            ...
            {false
              ? (<Loader />)
              : (<button type='button' onClick={handleSubmit} className="text-white w-full mt-2 border-[1px] p-2 border-[#3d4f7c] hover:bg-[#3d4f7c] rounded-full cursor-pointer">Send Now</button>)
            }
          </div>
        </div>
      </div>
